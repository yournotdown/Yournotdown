...
class TestAnalyticsContract(unittest.TestCase):
    ...
    def test_business_appearance_events_now_stamp_slot(self):
        ...
        self.assertIn('"itinerary generation failed: city=%s vibe=%s error_type=%s"', source)
    ...
    def test_hotel_qr_create_update_and_deactivate_return_json_safe_responses(self):
        ...
        self.assertIn('"hotel qr create failed: city_slug=%s error_type=%s"', create_source)
        self.assertIn('"hotel qr update failed: qr_id=%s error_type=%s"', update_source)
        self.assertIn('"hotel qr deactivate failed: qr_id=%s error_type=%s"', deactivate_source)
        self.assertIn('"hotel qr delete failed: qr_id=%s error_type=%s"', delete_source)
